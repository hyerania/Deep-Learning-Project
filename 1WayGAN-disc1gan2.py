...
#         print(fake_imgs.data)
        ...
        f = open("./models_d1g2/log_Train.txt","a+")
        f.write("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f]\n" % (epoch, NUM_EPOCHS_TRAIN , i, len(trainLoader_cross), dLoss.item(), gLoss.item()))
#         f.write("Validity:%f\n" % (gAdvLoss))
        f.close()

        if batches_done % 200 == 0:
            save_image(fake_imgs.data[:25], "./models_d1g2/images_train/1Way_Train_%d.png" % batches_done, nrow=5, normalize=True)
            torch.save(generator1.state_dict(), './models_d1g2/models_train/gan1_train_'+ str(epoch) + '_' + str(i) + '.pth')
            torch.save(discriminator.state_dict(), './models_d1g2/models_train/discriminator_train_'+ str(epoch) + '_' + str(i) + '.pth')
            fake_test_imgs = generator1(testInput)
            save_image(fake_test_imgs.data[:25], "./models_d1g2/images_test/1Way_Train_%d.png" % batches_done, nrow=5, normalize=True)

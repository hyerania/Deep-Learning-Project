...
for epoch in range(NUM_EPOCHS_TRAIN):
    for i, (data, gt1) in enumerate(trainLoader_cross, 0):
        ...
        f = open("./model/log_Train.txt","a+")
        f.write("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f]\n" % (epoch, NUM_EPOCHS_TRAIN , i, len(trainLoader_cross), dLoss.item(), gLoss.item()))
        f.close()

        if batches_done % 200 == 0:
            save_image(fake_imgs.data[:25], "./model/train_images/1Way_Train_%d.png" % batches_done, nrow=5, normalize=True)
            torch.save(generator1.state_dict(), './model/train_checkpoint/gan1_train_'+ str(epoch) + '_' + str(i) + '.pth')
            torch.save(discriminator.state_dict(), './model/train_checkpoint/discriminator_train_'+ str(epoch) + '_' + str(i) + '.pth')
            fake_test_imgs = generator1(testInput)
            save_image(fake_test_imgs.data[:25], "./model/train_test_images/1Way_Train_Test_%d.png" % batches_done, nrow=5, normalize=True)
...
with torch.no_grad():
    ...
    for j, (gt, data) in enumerate(testLoader, 0):
        ...
        psnrAvg += psnr
        save_image(output.data[:25], "./model/test_images/test_%d_%d.png" %(batches_done,j), nrow=5, normalize=True)
        save_image(realImgs.data[:25], "./model/gt_images/gt_%d_%d.png" %(batches_done,j), nrow=5, normalize=True)
        save_image(trainInput.data[:25], "./model/input_images/input_%d_%d.png" %(batches_done,j), nrow=5, normalize=True)
        batches_done += 5
        print("Loss loss: %f" % loss)
        print("PSNR Avg: %f" % (psnrAvg / (j+1)))
        f = open("./model/psnr_Score.txt","a+")

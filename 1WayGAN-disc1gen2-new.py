...
### Pre Training LOAD
generator1 = Generator()
generator1.load_state_dict(torch.load('./models/models_pretrain/gan1_pretrain_24_449.pth'))
generator1.eval()
...
#         print(fake_imgs.data)
        ...
        f = open("./models_new2_d1g2/log_Train.txt","a+")
        f.write("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f]\n" % (epoch, NUM_EPOCHS_TRAIN , i, len(trainLoader_cross), dLoss.item(), gLoss.item()))
#         f.write("Validity:%f\n" % (gAdvLoss))
        f.close()

        if batches_done % 200 == 0:
            save_image(fake_imgs.data[:25], "./models_new2_d1g2/images_train/1Way_Train_%d.png" % batches_done, nrow=5, normalize=True)
            torch.save(generator1.state_dict(), './models_new2_d1g2/models_train/gan1_train_'+ str(epoch) + '_' + str(i) + '.pth')
            torch.save(discriminator.state_dict(), './models_new2_d1g2/models_train/discriminator_train_'+ str(epoch) + '_' + str(i) + '.pth')
            #fake_test_imgs = generator1(testInput)
            #save_image(fake_test_imgs.data[:25], "./models_new2_d1g2/images_test/1Way_Train_%d.png" % batches_done, nrow=5, normalize=True)
            with torch.no_grad():
                psnrAvg = 0.0
                for j, (gt, data) in enumerate(testLoader, 0):
                    ...
                    psnrAvg += psnr
                    save_image(fake_test_imgs.data[:25], "./models_new2_d1g2/images_test/1Way_Test_%d%d.png" %(batches_done,j), nrow=5, normalize=True)
                    save_image(realImgs data[:25], "./models_new2_d1g2/ground_truths/gt_%d%d.png" %(batches_done,j), nrow=5, normalize=True)
                    save_image(trainInput.data[:25], "./models_new2_d1g2/inputs/input_%d%d.png" %(batches_done,j), nrow=5, normalize=True)
                    ...
                    f = open("./models_new2_d1g2/psnr_avg_Train.txt","a+")
                    f.write("PSNR Avg: %f" % (psnrAvg / (j+1))
                print("Final PSNR Avg: %f" % (psnrAvg / len(testLoader)))
                f = open("./models_new2_d1g2/psnr_avgfinal_Train.txt","a+")
                f.write("Final PSNR Avg: %f" % (psnrAvg / len(testLoader)))
        batches_done += 1
        print("Done training discriminator on iteration: %d" % (j))
